...
def reranker_parse_args():
    ...
    parser.add_argument('--reload_path', type=str,
                        default='./model//save_model_ad/10/202502062211_lambdaMART_NS_generator_512_0.0001_0.0001_64_16_0.8_1',
                        help='model ckpt dir')
...
if __name__ == '__main__':
    ...
    if params.model_type == 'CMR_generator' or params.model_type == 'CMR_PRM_generator' :
        ...
        model = NS_generator(num_ft, params.eb_dim, params.hidden_size, max_time_len, itm_spar_fnum,
                         itm_dens_fnum, profile_fnum, max_norm=params.max_norm, rep_num=params.rep_num,
    ...
    if not params.controllable:
        ...
        for i, s in enumerate(params.metric_scope):
            print("@%d  MAP: %.4f  NDCG: %.4f  CLICKS: %.4f  ILAD: %.4f  ERR_IA: %.4f  ALPHA_NDCG: %.4f  EVA_SUM: %.4f  EVA_AVE: %.4f" % (
                s, res[0][i], res[1][i], res[2][i], res[3][i], res[4][i], res[5][i], res[7][i], res[8][i]))
            # for i, s in enumerate(params.metric_scope):
